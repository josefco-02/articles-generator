...
def most_relevant_articles():
    ...
    contents = [
        types.Content(
            role="user",
            parts=[
                types.Part.from_text(text='Actúa como un agregador de noticias actualizado. Devuélveme las 6 noticias más relevantes del día de hoy en periódicos de España para cada una de las siguientes categorías: Internacional, Política, Sociedad, Economía, Deportes y Tecnología. Para cada noticia, proporciona únicamente un breve texto descriptivo que pueda servir como búsqueda semántica representativa del contenido de la noticia y que me ayude a buscar noticias relacionadas. Organiza toda la salida exclusivamente en formato JSON, sin saltos de línea, donde cada categoría sea una clave (en minúsculas y sin tildes) que contenga un array de 6 textos semánticos. No añadas explicaciones, texto fuera del JSON ni ningún tipo de número, enlace, fuente o referencia entre corchetes, paréntesis o de ningún tipo. Solo un JSON con el texto limpio.'),
            ],
        ),
    ]
    tools = [
